...
local warned
local globalLoaded
RayfieldLibrary.rayfieldDestroyed = false -- True when RayfieldLibrary:Destroy() is called
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if RayfieldLibrary.rayfieldDestroyed then
					return
				end
				if not Success then
...
function RayfieldLibrary.AddDestroyCallback(callback)
	table.insert(DestroyCallbacks, callback)
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RayfieldLibrary:Destroy()
	RayfieldLibrary.rayfieldDestroyed = true
